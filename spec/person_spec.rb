...
describe Person do
  describe '#instance' do
    ...
    context 'with having all parameters' do
      person = Person.new(name: 'Gold', age: 14, parent_permission: false)
  ...
  describe '#can_use_services' do
    context 'when person is below 18 years' do
      it 'should not be allowed' do
        person = Person.new(name: 'Grey', age: 13, parent_permission: false)
        expect(person.can_use_services?).to be_falsey
      end

      it 'should be allowed only with parent permission' do
        person = Person.new(name: 'Grey', age: 13, parent_permission: true)
        expect(person.can_use_services?).to be_truthy
      end
    end

    context 'when person is above 18 years' do
      person = Person.new(name: 'Grey', age: 18, parent_permission: false)
